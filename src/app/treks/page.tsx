...
function buildOptions<V extends string>({
    ...
    for (const v of candidates) {
        const count = treks.filter((t) => matches(t, v)).length;
        if (count === 0) continue;
        options.push({
            value: v,
            label: labelOf(v),
...
export default async function TreksPage({ searchParams }: TreksPageProps) {
    ...
    const regionOptions: SidebarOption[] = REGIONS.map((r) => ({
        ...
        count:
            r.value === "all"
                ? treks.length
                : treks.filter((t) => t.region === r.value).length,
    ...
    for (const t of treks) for (const m of monthsCovered(t.bestMonths)) monthSet.add(m);
    const monthOptions: SidebarOption[] = MONTHS.filter((m) => monthSet.has(m)).map(
        (m) => ({
            value: m,
            label: MONTH_FULL[m] ?? m,
            count: treks.filter((t) => monthsCovered(t.bestMonths).includes(m)).length,
